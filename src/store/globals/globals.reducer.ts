...
console.log('lastGameInLS', lastGameInLS?.activeField);
...
export const globalsReducer = createSlice({
    ...
    reducers: {
        setScore: (state, { payload }: PayloadAction<{ status: TStatus }>) => {
            const currentLSValue = state.results;
            const newLSValue = {
                win: payload.status === 'win' ? (currentLSValue?.win || 0) + 1 : currentLSValue?.win || 0,
                lose: payload.status === 'gameover' ? (currentLSValue?.lose || 0) + 1 : currentLSValue?.lose || 0,
            };
...
export const { setScore, setGameOverMsg, setActiveField, saveActiveGame, setLastActiveMemoryField } =
    globalsReducer.actions;
